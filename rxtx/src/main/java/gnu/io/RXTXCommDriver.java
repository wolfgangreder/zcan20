...
public class RXTXCommDriver implements CommDriver
{
  ...
  static {
    ...
    RXTXVersion.loadNatives("rxtxSerial");
...
//    }
        ...
        for (p = 0; p < ValidPortPrefixes.length; p++) {
          ...
          if (osName.equals("Solaris") || osName.equals("SunOS")) {
            checkSolaris(PortName,
                         PortType);
          } else if (true || testRead(PortName,
                                      PortType)) {
            CommPortIdentifier.addPortName(
                    PortName,
                    PortType,
  ...
  private boolean registerSpecifiedPorts(int PortType)
  {
    ...
    if (props == null) {
      ...
      try {
        ...
        if (debug) {
          System.out.println(
                  "RXTXCommDriver.registerSpecifiedPorts(): Error encountered while getting gnu.io.rxtx.properties from the classpath: "
                  + e);
        }
      }
    }
    ...
    if (props == null) {
      if (debug) {
        System.out.println(
                "RXTXCommDriver.registerSpecifiedPorts(): RxTx ports not specified in properties file, checking system properties... ");
      }
      final String serialProp = System.getProperty("gnu.io.rxtx.SerialPorts",
                                                   null);
      final String parallelProp = System.getProperty("gnu.io.rxtx.ParallelPorts",
                                                     null);
      if (serialProp != null || parallelProp != null) {
        props = new Properties();
        if (serialProp != null) {
          if (debug) {
            System.out.println("RXTXCommDriver.registerSpecifiedPorts(): Found gnu.io.rxtx.SerialPorts property with value of ["
                                       + serialProp + "]");
          }
          props.setProperty("gnu.io.rxtx.SerialPorts",
                            serialProp);
        }
        if (parallelProp != null) {
          if (debug) {
            System.out.println("RXTXCommDriver.registerSpecifiedPorts(): Found gnu.io.rxtx.ParallelPorts property with value of ["
                                       + parallelProp + "]");
          }
          props.setProperty("gnu.io.rxtx.ParallelPorts",
                            parallelProp);
        }
      }
    }
    ...
    if (props != null) {
      ...
      if (debug) {
        System.out.println(
                "RXTXCommDriver.registerSpecifiedPorts(): The file: gnu.io.rxtx.properties doesn't exist and the properties weren't specified as a system property.");
      }
    }
    if (val != null) {
 ...
 /*
   ...
   * @return CommPort getCommPort() will be called by CommPortIdentifier from its openPort() method. PortName is a string that was
   * registered earlier using the CommPortIdentifier.addPortName() method. getCommPort() returns an object that extends either
